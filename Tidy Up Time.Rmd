...
to then create a plot of the probability of active moult in female and male sunbirds and the probability of a brood patch being present. The species code (SPP) can be changed to pull data different species. 
...
The packages that have been installed then need to be loaded. 

```{r step 3, results='hide', warning=FALSE}
library(dplyr)
...
The 'NAs' then need replaced with 'OK' to soften the next filter step.
